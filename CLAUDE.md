...
6. **Plain words everywhere.** Every explanation and concept card must be understandable by someone who has never heard the term before. Define jargon on first use. Prefer "how fast the work grows when the input grows" over "asymptotic complexity" — then introduce the formal term.
7. **Mobile first, literally.** Develop and test at 390×844 (iPhone-ish) viewport. Desktop is a centered ~480px column. Touch targets ≥ 44px. No hover-dependent interactions.
8. **Verify before committing:** `npm run build` and `npm run lint` must pass; test the interaction you just built in the browser at mobile viewport.
