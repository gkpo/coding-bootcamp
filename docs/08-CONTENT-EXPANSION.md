...
| t9-03 | mcq      | 2    | Read a GROUP BY + HAVING query: what one result row *means*                                                                                | sql-reading  |
...
| t9-08 | mcq      | 2    | Fix t9-07: one query with `IN` / a JOIN (or the ORM's include), and how to *say* the diagnosis in an interview                              | n-plus-one   |
