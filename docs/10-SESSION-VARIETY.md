...
3. **Decoder slot rotates.** Prefer unseen decoder items in authored order; when all are seen, pick a _due_ one if any; otherwise rotate deterministically by day (index derived from the day key) so it changes daily instead of pinning to the first unmastered.
...
- Authoring rule: variants are for exercises whose prompt is a _problem statement or riddle_ (statement-to-pattern mcqs, decoder-adjacent mcqs). Mechanics drills (parsons, blank, spot-bug) do not need them; their repetition is the point.
...
| Decoder depth   | t6       | 8     | match + mcq    | 5 new match exercises pairing _unused_ phrasings of already-covered concepts (per the docs/09 rule: never reuse a pairing text verbatim), and 3 mcqs decoding longer interviewer sentences ("if I gave you a sorted copy, would that help?").                                                                                                                                                                                                   |
