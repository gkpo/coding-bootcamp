...
| `--success`                                  | `#49970A`                                | Correct answers, mastered checks. The accent's own grass green: kept apart from the accent by role alone (see below)           |
| `--danger`                                   | `#C63D2F`                                | Wrong answers. Brick red, picked to sit with the green (used gently. Border/tint, never full red panels)                       |
| `--info`                                     | `#2F6098`                                | Muted denim blue. Links, ghost-button labels, the session bar, the concept chip's help mark, focus and selection outlines      |
...
`--success` sat at teal for one release, on the rule that it had to stay clearly apart from `--accent` or "correct" would read as "the accent again". **That rule is retired.** The near-accent leaf green was put beside the accent, the collision was shown, and it was chosen anyway. Success then moved on to sea green and danger back to its original pink red, both picked the same way, and neither move brought the old rule back: the doc records the choices rather than the reasoning they replaced.

Today success has gone the whole way. **`--success` and `--accent` are the same hex, `#49970A`**, chosen by the user off the grass green of the practiced tiles in the streak week. There is no hue distance left between "correct" and "the accent", and none is wanted. **Role is the only separator:**
...
Two colours that never share a component do not blur, because nothing ever puts them side by side and asks the eye to tell them apart. That is now the entire argument, so keep the roles clean: the moment success fills something you press, or the accent outlines something you have just been graded on, the palette stops working. Track colours still avoid green, for the separate reason given below.

`--danger` and `--info` were picked to sit with that green rather than against it: brick red `#C63D2F` and muted denim `#2F6098`. Grass green and brick red are an earthy pair, and denim completes the muted triad where a brighter blue would sit outside the set. `--info` is the one semantic colour that carries real text (links, ghost-button labels, the session bar), so it is held to the 4.5:1 text bar on all three surfaces (6.46 on `--surface`, 5.92 on `--bg`, 5.46 on `--surface-2`) rather than the 3:1 the other two get as borders and glyphs.

The second half of that: a state colour drawn **as a box outline around a state-tinted fill** takes the soft step (`--success-soft`, `--danger-soft`, `--info-soft`, each the full colour at 70% into `--surface`). A state colour that **is the mark** stays full strength: the filled radio ring, a chip fill, coloured text, a drawn glyph, the 3px edge on a concept-card note or a code block, and any outline that means attention rather than decoration (a capstone board error). On `--surface` the three resolve to `#80B654` (2.40), `#D7776D` (3.11) and `#6D90B7` (3.32), so danger and info sit just over the 3:1 line and success sits clearly under it: the grass green is the lightest of the three to begin with, and 30% white takes it further. That is tolerable for this one job because the outline is not carrying the verdict; the ring, the tint and the words all state it at full strength first, and the outline is only drawing the edge of the box they sit in. Drawing that edge at full strength doubles the same signal and turns a graded answer into a warning label.
...
- **Streak flame:** the one playful mascot-ish element. Inline SVG flame, accent-colored, subtle 1.5s ease pulse when today is done; grayed at `--text-dim` when today isn't done yet.
  The practiced days in the streak week are filled with the accent tint (`--accent-bg`, the accent at 16%). Since success and the accent are one hex, that tile and `--success-bg` are literally the same colour at two strengths, 16% and 10%. That is accepted: the two never appear in the same component, or on the same screen. The streak week lives on Home and the success tint lives on a graded exercise, so no one ever sees them together and has to decide which is which.
