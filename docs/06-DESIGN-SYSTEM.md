...
| `--accent-text` | `#B45309` | Accent used *as text or small icons* on light bg (the fill amber fails AA as text — always use this darker step for text/strokes) |
...
- **Code block:** `--surface-2`, 12px radius, 12px padding, mono 13.5, highlight theme matched to this palette (tune a Shiki/hljs *light* theme's bg to `--surface-2`; keep token colors in the deep range of the track colors — no neon-on-white).
