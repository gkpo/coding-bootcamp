...
| `--info`                                     | `#12629E`                                | Links, the concept chip's icon and label                                                                                         |
| `--success-bg` / `--danger-bg` / `--info-bg` | the above at ~10% opacity on `--surface` | Feedback panel tints                                                                                                             |
| `--info-edge`                                | `--info` at 28% opacity                  | The concept chip's hairline ring and the underside it presses into. An edge, never a fill and never text                         |
...
- **Chips:** concept chip = the drawn help mark at 20px plus the words "Concept card", on `--info-bg` with `--info` text, a 1px `--info-edge` ring and a 3px `--info-edge` underside, radius full, min-height 44px. It presses into its underside like the primary button, one third of the travel. **The label names the object, never the concept:** the concept's own name is the answer on "name the pattern" exercises, so no chip ever carries it. Option pills for parsons/blank = `--surface-2` + border, drag state lifts with slight scale + border-accent.
