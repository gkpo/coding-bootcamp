...
- Wrong options are *plausible misconceptions*, each with a `whyWrong` where the schema allows.
...
Goal: see code → name its growth → *say it* the way interviewers expect. Directly targets the "it grows linearly" failure.
...
| t1-05 | complexity | 2 | Two *sequential* loops → still O(n) (not O(2n), not O(n²)) | big-o |
...
| t2-04 | mcq | 2 | When greedy coin change *fails* (denominations 1,3,4 for amount 6) → why DP exists | greedy |
...
| t2-24 | mcq | 3 | Trick round: a statement that *looks like* sliding window but is a frequency map — reading carefully | pattern-map |
...
| t4-06 | ladder | 2 | Works but O(n²): interviewer asks "what next?" — correctness tests *before* optimizing | refactor-ladder |
...
| t4-09 | ladder | 3 | The already-written-bad-function interview: first *say* what's wrong (it grows linearly / it re-reads the array), then fix, then types — order the critique | refactor-ladder |
...
| perf-script | Talking about performance | 1/6 | — (it *is* the script) |
...
*(Count note: a few cards serve two tracks; total distinct cards ≈ 45 in this table — treat the table as authoritative rather than the "35" estimate. All must exist because exercises link to them.)*
